...
function PuzzleSidebar() {
  ...
  const onDragStart =
    ...
    (event: DragEvent<HTMLDivElement>) => {
      ...
      const y = event.clientY - node.offsetTop + (ref.current?.scrollTop || 0);
      const distance = {
        x,
        y,
      };

      event.dataTransfer.setData("application/react-cosmos-diagram", nodeType);
      event.dataTransfer.setData("application/node", JSON.stringify(distance));
