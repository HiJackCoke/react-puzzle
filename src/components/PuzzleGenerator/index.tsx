...
const PuzzleGenerator = ({ onImageUpdate }: Props) => {
  ...
  const splitImage = (img: HTMLImageElement) => {
    ...
    const pieceSize = Math.floor(Math.min(img.width / cols, img.height / rows));
    ...
    for (let row = 0; row < rows; row++) {
      ...
      for (let col = 0; col < cols; col++) {
        const top = row === 0 ? "flat" : invert(pieceMap[row - 1][col].bottom);
        const left = col === 0 ? "flat" : invert(pieceMap[row][col - 1].right);
